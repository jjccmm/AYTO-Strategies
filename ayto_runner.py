...
def create_plots(df):
    ...
    for i, solver_name in enumerate(['random', 'paper_random', 'paper_clever', 'pc_random', 'pc_max', 'pc_minmax']):  # df['solver'].unique()
        ...
        percentage_solved = df_solved['run'].nunique() / (df['run'].max() + 1)
        ...
        ax[7].yaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{v / 100:.0f}k€"))
